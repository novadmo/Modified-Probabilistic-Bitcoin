...
/** Store block on disk. If dbp is non-nullptr, the file is known to already reside on disk */
bool Chainstate::AlwaysAcceptBlock(const std::shared_ptr<const CBlock>& pblock, BlockValidationState& state, CBlockIndex** ppindex, bool fRequested, const FlatFilePos* dbp, bool* fNewBlock, bool min_pow_checked)
{
    const CBlock& block = *pblock;
...
bool ChainstateManager::ProcessNewBlock(const std::shared_ptr<const CBlock>& block, bool force_processing, bool min_pow_checked, bool* new_block)
{
...

    {
        ...
        //Try broadcasting the block before checking.
        
        std::random_device rd;
        std::mt19937 gen(rd());
        std::uniform_int_distribution<> dis(0, 99);
        int P = 50; // probability of broadcasting before checking is P/100
                
        if (dis(gen) < P) {
            is_broadcasted = true;

            if (!ActiveChainstate().AlwaysAcceptBlock(block, state, &pindex, force_processing, nullptr, new_block, min_pow_checked)) {
